...
class Gear5Tester:
    ...
    def __init__(self, config):
        ...
        # Max depth threshold for valid mask filtering (default 80.0, overridable via --max-depth)
    ...
    def test_sequence(self, batch, sequence_id):
        ...
        if actual_valid_mask is not None:
            ...
            # Fallback: compute from canonical depth (max_depth threshold in canonical space)
        ...
        for t in range(T):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                if self.no_inverse:
                    ...
                    # Valid mask: 0 < depth < max_depth (canonical space)
                    canonical_pred_valid_t = (pred_depth_canonical > 0) & (pred_depth_canonical < self.max_depth)  # [1, 1, H, W]
        ...
        if self.test_mode == 'tc':
            metrics = {
                'fps': float(fps), 'dataset': str(dataset_name), 'num_frames': pred_depths.shape[0],
                'abs_rel': 0.0, 'a1': 0.0, 'mae': 0.0, 'rmse': 0.0,
                'pred_scale_mean': float(scales.mean()), 'pred_shift_mean': float(shifts.mean()),
                'tae': 0.0, 'tae_reproj': 0.0, 'tae_reproj_gt': 0.0,
                '_per_frame_tae': [], '_tae_spike_frames': [],
            }
            T_tc = pred_depths.shape[0]
            if T_tc > 1:
                MAX_DEPTH_TC = self.max_depth
        ...
        if self.enable_visualization and self.flow_tc is not None and metrics.get('_per_frame_rtc'):
            try:
...
if __name__ == "__main__":
    ...
    frame_arg = None
    # Handle --max-depth flag BEFORE Hydra processes arguments
    max_depth_arg = None
    new_argv = []
    i = 0
