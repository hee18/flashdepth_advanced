...
class Gear4Visualizer:
    ...
    def create_validation_summary(self, sample_batch, model_outputs, step, save_name=None, prefix="validation", fps=None, loss_dict=None, dataset_name=None, layer_weights=None, config=None):
        """
        ...
        try:
            ...
            if fg_mask is not None:
                ...
                if fg_mask.ndim == 4:
                    fg_mask_frame = fg_mask[0, 0].float().cpu().numpy()
                else:
                    fg_mask_frame = fg_mask[0].float().cpu().numpy()
            else:
                fg_mask_frame = None
            ...
            if bg_mask is not None:
                ...
                if bg_mask.ndim == 4:
                    bg_mask_frame = bg_mask[0, 0].float().cpu().numpy()
                else:
                    bg_mask_frame = bg_mask[0].float().cpu().numpy()
            else:
                bg_mask_frame = None
            ...
            if 'canonical_gt_valid' in model_outputs:
                ...
                if is_sparse:
                    ...
                    # 3. Final pred visualization mask:
                    #    Within valid height AND (GT valid OR (GT missing AND Pred valid))
