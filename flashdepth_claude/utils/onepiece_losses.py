...
3-loss system:
    - L_log_l1: Full graph metric depth loss (Mamba + FiLM + MetricHead + RelativeHead)
    - L_tgm: Full graph temporal gradient matching
    - L_ofc: Optical Flow Consistency loss, FiLM→Mamba gradient (Phase 2 only)
...
class OpticalFlowConsistencyLoss(nn.Module):
    """
    Optical Flow Consistency (OFC) Loss.
...
class OnepieceCombinedLoss(nn.Module):
    ...
    LogL1 uses full-graph metric_depth (gradient flows to all trainable modules).
    """

    def __init__(self, log_l1_weight=1.0, tgm_weight=1.0,
                 ofc_weight=0.01, use_log_space=True):
        super().__init__()
    ...
    def forward(self,
        ...
        Args:
            metric_depth: [B, T, H, W] full-graph metric depth (for LogL1 + TGM)
            gt_depth: [B, T, H, W] ground truth (inverse depth)
            valid_mask: [B, T, H, W] validity mask
            modulated_features: [B, T, 256, h, w] for OFC (Phase 2)
        ...
        """
        # 1. Log L1 (always, full graph)
        l_logl1 = self.log_l1_loss(metric_depth, gt_depth, valid_mask)

        # 2. TGM (always, full graph)
        ...
        # 3. OFC (Phase 2 only, FiLM→Mamba gradient)
        if (phase == 2 and self.ofc_weight > 0 and
                modulated_features is not None and
                images is not None and
                flow_estimator is not None and
                metric_depth.shape[1] >= 2):
            l_ofc = self.ofc_loss(modulated_features, images, flow_estimator)
        else:
            l_ofc = torch.tensor(0.0, device=metric_depth.device)
        ...
        if phase == 2:
            total_loss = total_loss + self.ofc_weight * l_ofc
