            ...
            if (step in vis_steps or step % 250 == 0) and self.train_visualizer and self.rank == 0:
                try:
                    ...
                    with torch.no_grad():
                        ...
                        gt_depth_metric = 100.0 / (gt_t_inverse_100 + 1e-8)
        ...
        for batch_idx, batch in enumerate(tqdm(self.val_loader, desc="Validation", disable=(self.rank != 0))):
            ...
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                for t in range(T):
                    ...
                    gt_t_shape = gt_t_inverse_100.shape[-2:]  # GT original resolution
                ...
                # Track sequence count (regardless of valid pixels)
            ...
            # Increment sequence counter for visualization
