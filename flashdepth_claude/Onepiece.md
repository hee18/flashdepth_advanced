...
Onepiece V2는 FlashDepth의 relative depth를 metric depth로 변환하기 위한 통합 아키텍처. V1 대비 핵심 변경: **CLS token 제거**, GAP+GStdP 기반 512-dim 입력, FiLM/MetricHead 분리, 4-loss gradient isolation, SceneCutDetector 삭제.
...
| Loss 체계 | LogL1 + TGM + WFC (3-loss) | **LogL1 + TGM + WFC + SSIL (4-loss, gradient isolation)** |
...
spatial_mean → [B*T, 2]
    |                           |
    v                           v
softplus(raw_scale)          1.0 * sigmoid(raw_shift)
= scale (always positive)   = shift (range [0, 1.0])
```
...
**Default weights**: `1:1:0.01:1` (WFC raw L2 스케일이 크므로 0.01로 축소)

### Gradient Isolation Strategy

| Loss | Target | Gradient Scope | Phase | Implementation |
|------|--------|---------------|-------|----------------|
| **Log L1** | metric_depth_isolated | MetricHead only | 1+2 | `metric_head(modulated.detach())`, `rel.detach()` |
| **TGM** | metric_depth (full graph) | Both Heads + FiLM + Mamba | 1+2 | Normal forward |
| **WFC** | modulated_features | FiLM → Mamba | 2 only | Normal on modulated features |
| **SSIL** | relative_depth_isolated | RelativeHead only | 2 only | `rel_head(modulated.detach())` |

### 1. Log L1 Loss (MetricHead gradient only)
...
- Gradient flows through MetricHead only (modulated features detached)
...
- No scene cut weighting (SCD removed)

### 3. Warp Feature Consistency Loss (FiLM→Mamba, Phase 2 only)
...
- Flow confidence는 WFC 내부에서만 사용 (다른 loss에 전달 안 함)

### 4. SSIL Loss (RelativeHead gradient only, Phase 2 only)

Scale-and-shift invariant loss. RelativeHead의 depth 품질을 독립 개선.

- Uses `ScaleAndShiftInvariantLoss` from `flashdepth/util/loss.py`
- GT: inverse depth (1/m) * 100 (relative depth output scale에 맞춤)
- Gradient flows through RelativeHead only (modulated features detached)
- **Shape 주의**: `compute_scale_and_shift`가 dims `(1, 2)` = (H, W) sum 기반 per-sample alignment이므로, 입력은 반드시 3D `[N, H, W]`. `onepiece_losses.py`에서 `[B, T, H, W]` → `[B*T, H, W]` reshape 후 전달
- **Mask 타입**: SSIL 내부에서 boolean indexing 사용하므로 mask는 `.bool()` 변환 필요 (float mask 불가)

### Loss Shape Convention

각 로스 함수가 기대하는 입력 shape:
...
| **WFC** | `[B, T, C, h, w]` (5D) | 내부 `B, T, C, h, w = shape` 파싱 | features + images 모두 5D |
| **SSIL** | `[N, H, W]` (3D) | `onepiece_losses.py`에서 reshape | `compute_scale_and_shift`가 dims (1,2) sum |

`train_onepiece.py`의 `train_step`에서 model output → loss 전달 시:
- `metric_depth`, `gt_depth`, `valid_mask` → `[B, T, H, W]` 그대로 전달 (LogL1, TGM이 처리)
- `relative_depth_isolated`, `gt_depth_for_ssil` → `onepiece_losses.py`에서 `[B*T, H, W]`로 reshape
- `relative_depth_isolated`도 GT와 해상도 불일치 시 `F.interpolate` resize 필요 (`train_step` 내 처리)
...
- **Phase 2 훈련 시에만 사용**, Phase 1 및 추론/테스트에서는 불필요
...
**Sea-RAFT 없이도 Phase 1 훈련 가능** (WFC는 Phase 2에서만 활성화). 테스트는 항상 가능.
...
| OnepieceFiLMGenerator | **Frozen** |
| UnifiedGlobalMamba | **Trainable** |
| OnepieceMetricHead | **Trainable** |

- Losses: **Log L1 + TGM** (WFC, SSIL skipped)
...
- Losses: **All 4** (Log L1 + TGM + WFC + SSIL)
...
6b. **MetricHead**: `modulated → scale, shift`
7. **Metric conversion**: `metric_depth = scale * (1/relative_depth) + shift`
8. **Gradient isolation**: `modulated.detach()` → isolated forward for LogL1 and SSIL

### Return Dict

```python
{
    'metric_depth': [B, T, H, W],            # Full graph (for TGM)
    'relative_depth': [B, T, H, W],          # Full graph
    'metric_depth_isolated': [B, T, H, W],   # MetricHead only (for LogL1)
    'relative_depth_isolated': [B, T, H, W], # RelativeHead only (for SSIL, Phase 2)
    'modulated_features': [B, T, 256, h, w], # For WFC
    'scale': [B, T],                          # Per-frame scale (init ≈ 100)
...
├── utils/
│   ├── onepiece_losses.py          # WarpFeatureConsistencyLoss, OnepieceCombinedLoss (4-loss)
...
- **Feature downsample**: WFC uses full-resolution modulated features (no 1/4 downsampling).
