"""
Gear3 Hybrid Training Script: Feature-level Metric Depth Learning with Teacher-Student Fusion

This is the 2K resolution version using FlashDepth Full architecture:
    - Teacher: FlashDepth-L (frozen) - processes 518x518 downsampled image
    - Student: FlashDepth-S (frozen encoder, trainable head) - processes 2K image
    - Hybrid Fusion: Cross-Attention (frozen) - combines teacher/student path_4
    - Gear3 Modulation: Applied to fused DPT features
    - Mamba: Trained from scratch on modulated features
...
Key differences from baseline:
    ...
    - Feature-level modulation using FiLM (after hybrid fusion)
...
class Gear3Trainer:
    ...
    def _setup_model(self):
        """Initialize FlashDepth Hybrid (teacher-student) with Gear3 metric head"""
        # Create student model (FlashDepth-S for 2K)
        model_config = dict(self.config.model)
        model_config['batch_size'] = self.config.training.batch_size
        model_config['use_metric_head'] = False  # Don't use GSP head
        model_config['vit_size'] = 'vits'  # Student uses ViT-S

        # Add hybrid configs for teacher-student fusion
        model_config['hybrid_configs'] = dict(self.config.hybrid_configs)

        model = FlashDepth(**model_config)

        self.logger.info(f"Created student model (FlashDepth-S) with hybrid fusion")
        ...
        if checkpoint_path and checkpoint_path != 'true':
            if os.path.exists(checkpoint_path):
                ...
                # Load student (F-S), teacher (F-L), and hybrid_fusion
                # Exclude only: Mamba, output_conv1/2 (modulated features)
                loaded_dict = {}
                excluded_keys = []
                for k, v in state_dict.items():
                    # Exclude modules that receive modulated features
                    if any(x in k for x in ['mamba', 'output_conv1', 'output_conv2']):
                        excluded_keys.append(k)
                    else:
                        loaded_dict[k] = v

                # Load state dict (strict=False to allow missing Mamba/output_conv)
                model.load_state_dict(loaded_dict, strict=False)
                self.logger.info(f"Loaded {len(loaded_dict)} parameters from checkpoint")
                self.logger.info(f"  - Student (DINOv2-S + DPT-S): ✓")
                self.logger.info(f"  - Teacher (DINOv2-L + DPT-L): ✓")
                self.logger.info(f"  - Hybrid Fusion (Cross-Attention): ✓")
                self.logger.info(f"Excluded {len(excluded_keys)} parameters for training:")
                self.logger.info(f"  - Mamba (modulated features)")
                self.logger.info(f"  - output_conv1/2 (modulated features)")
            else:
                self.logger.warning(f"Checkpoint {checkpoint_path} not found")

        # Add Gear3 metric head (student is ViT-S)
        embed_dim = 384  # Student uses ViT-S
        dpt_dim = 64     # Student uses DPT-S
        num_heads = 6    # ViT-S has 6 attention heads
        ...
        self.logger.info(f"Added Gear3 head for student model (ViT-S, dpt_dim={dpt_dim})")
    ...
    def _configure_parameters(self, model):
        """
        Freeze: Student/Teacher encoders, DPT refinement, Hybrid fusion
        ...
        for name, param in model.named_parameters():
            if 'gear3_head' in name:
                ...
                # Mamba: train from scratch (receives modulated features)
                param.requires_grad = True
                ...
                # - Student (DINOv2-S, DPT-S refinement)
                # - Teacher (DINOv2-L, DPT-L)
                # - Hybrid Fusion (Cross-Attention)
                param.requires_grad = False
                frozen_params += param.numel()

        self.logger.info(f"Frozen (Student, Teacher, Hybrid Fusion): {frozen_params:,}")
...
@hydra.main(version_base=None, config_path="configs/gear3-hybrid", config_name="config")
