...
from utils.visualization import generate_checkpoint_visualization
...
def validation(cfg, model, train_step, test_dataloader):
    ...
    if rank == 0:
        for dataset_name, losses in val_losses.items():
            abs_rel = losses.get('abs_rel')
            delta1 = losses.get('delta1')
            logging.info(
                f"Validation step {train_step} | {dataset_name}: "
                f"AbsRel={abs_rel:.4f} Delta1={delta1:.4f}" if abs_rel is not None and delta1 is not None else
                f"Validation step {train_step} | {dataset_name}: metrics={losses}"
            )
        logging.info(
            f"Validation step {train_step} | avg: "
            f"AbsRel={avg_losses.get('abs_rel', float('nan')):.4f} "
            f"Delta1={avg_losses.get('delta1', float('nan')):.4f}"
        )
...
def inference(cfg, process_dict):
    ...
    for test_idx, batch in enumerate(tqdm(test_dataloader)):        
        ...
        img_grid = result.get('grid')

        if cfg.eval.save_grid and img_grid is not None:
            grid_image = img_grid.get('grid_img') if isinstance(img_grid, dict) else img_grid
            grid_image.save(f'{savepath}/{os.path.basename(cfg.config_dir.rstrip("/"))}_{train_step}_{test_idx}.png')
...
def main(cfg, process_dict):
    ...
    if process_dict['rank'] == 0:
        try:
            vis_video, vis_depth, vis_name = test_dataset[0]
            visualization_sample = {
                'video': vis_video.unsqueeze(0),
                'depth': vis_depth.unsqueeze(0),
                'name': vis_name if isinstance(vis_name, str) else str(vis_name),
                'frame_idx': vis_video.shape[0] // 2,
            }
        except Exception as exc:
            logging.warning(f"Unable to prepare visualization sample: {exc}")
    ...
    for train_idx in range(total_iters-train_step):
        ...
        if train_idx!=0 and train_idx % cfg.training.save_freq == 0:
            ...
            if process_dict['rank'] == 0:
                generate_checkpoint_visualization(model.module, visualization_sample, cfg, train_step)
            dist.barrier()
