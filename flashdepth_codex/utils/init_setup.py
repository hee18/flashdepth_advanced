...
def setup_model(cfg, process_dict):
    ...
    if not cfg.inference:
        ...
        gsp_lr = getattr(cfg.training.lr, 'gsp', 0.0)
        if getattr(model, 'metric_head_enabled', False) and gsp_lr:
            for param in model.gsp_head.parameters():
                param.requires_grad = True
            gsp_params = {"params": model.gsp_head.parameters(), "lr": gsp_lr}
            optim_list.append(gsp_params)
        ...
        if len(optim_list) == 0:
            raise ValueError("Optimizer parameter list is empty. Enable at least one learning rate component.")
